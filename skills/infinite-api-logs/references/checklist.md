...
- [ ] Todas as mensagens usam template com placeholder nomeado — nenhuma interpolação `$"..."`,
      concatenação ou `{0}`.
...
- [ ] `BackgroundService`/consumer abre `BeginScope` com o identificador do ciclo.
- [ ] Nada fora do escopo foi tocado: sem mudança de contrato, de regra de negócio, de DI, de Consul
      ou de pacote.
- [ ] `dotnet build src/Infinite.{{Servico}}.slnx` passa.
